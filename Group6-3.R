...
# Function to extract articles of a particular Year (Volume)
crawl_article <- function(year){
    ...
    # Function to get article URL and article publish date
    ...
    # Function to extract required data from individual articles
    Analysis_of_article = function(DATE, url){
          ...
          fullArticle = paste("https:",doc %>% html_nodes("#main-content > div > aside > div.c-pdf-download.u-clear-both > a") %>% html_attr("href"),sep = "")
          ...
          extract_data_from_row = data.frame(DATE, title, author, authorAffiliation, correspondingAuthor,
                                             correspondingAuthorEmail, publicationDate, abstract, keywords, fullArticle)
    ...
    extracted_data = data.frame("DOP"=c(),"Title"=c(), "Author"=c(), "Author_Affiliation"=c(), "Corresponding_Author"=c(), "Corresponding_Author_email"=c(), 
                                "Publication_Date"=c(), "Abstract"=c(), "Keywords"=c(), "Full Article"=c())
    total_number = as.integer(length(articles_data[,1]))
    
    for(i in 1:total_number){
      cat("\r",paste("Crawling data from article ",i,"..."))
...
#Enter year of which articles we want to extract.
crawl_article(2019)
